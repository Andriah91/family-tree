...
controllers.controller('register', ['$scope', '$location', '$http', 'alerts', 'user', function($scope, $location, $http, alerts, user) {

    $scope.alerts = alerts;
    $scope.input = {};
    $scope.user = user.getUser();

    $http.post('api/member/getName').success(function(data){
        $scope.Parents = data;
        console.log(data);
    });
    ...
    $scope.register = function() {
        ...
        if ($scope.input.email != $scope.input.confirmail) {
            alerts.fail(i18n.t('emails_not_match'));
            $scope.waiting = false;
            return;
        };

        $http.post('api/member/register', {
            parent : $scope.input.parent,
            firstname: $scope.input.fname,
            lastname: $scope.input.lname,
            nickname: $scope.input.nname,
            sex: $scope.input.sex,
            birthday: $scope.input.birth,
            deathday: $scope.input.death,
            profession: $scope.input.profession,
            email: $scope.input.email,
            phone: $scope.input.phone,
            address: $scope.input.address,
            story: $scope.input.story,
            img : $scope.input.img
        ...
        if ($scope.user.email == null){
            ...
            alerts.success(i18n.t('add_success'));
            $location.path('arbo');
        }
...
controllers.controller('arbo', ['$scope', '$location', '$http', 'user', function($scope, $location, $http, user) {

    var json = [
        {id:1 , v:44, f:[{id:2 , f:[{id:4,f:[{id:7,f:[{id:9,f:[{id:10}]}]}]},{id:5,f:[{id:8}]}]},{id:3},{id:6}]}
    ];
    $scope.Families = json;
